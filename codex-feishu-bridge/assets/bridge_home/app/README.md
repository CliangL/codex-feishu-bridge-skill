...
- Feishu keeps its own model selection in `~/.codex-feishu/codex-home/config.toml` and `~/.codex-feishu/feishu-model.json`, so desktop model switches do not affect the bot. Use `/model` and `/model <name>`.
