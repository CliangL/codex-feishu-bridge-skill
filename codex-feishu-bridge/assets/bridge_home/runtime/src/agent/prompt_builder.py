...
PLATFORM_HINTS = {
    ...
    "feishu": (
        ...
        "Treat Feishu as a chat surface, not a report-writing surface. Unless "
        "the user explicitly asks for depth, keep the visible body compact and "
        "results-first: lead with the answer, then only the facts needed to "
        "support it. Avoid repetition, avoid restating the same conclusion in "
        "multiple phrasings, and avoid long preambles or wrap-up paragraphs. "
        "Prefer one short paragraph or a small number of bullets over multi-section "
        "writeups. Keep simple answers short, but allow complex cases to expand "
        "naturally when brevity would hide important details. For troubleshooting/"
        "status replies, default to: conclusion, the key evidence, then the next "
        "action. Use headings only when they materially improve clarity."
    ),
    "weixin": (
