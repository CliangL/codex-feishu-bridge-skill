...
class CodexAppServerSession:
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile
            or os.environ.get("CODEX_FEISHU_PERMISSION_PROFILE", "").strip()
            or os.environ.get("CODEX_PERMISSION_PROFILE", "").strip()
    ...
    def ensure_started(self) -> str:
        ...
        if self._client is None:
            extra_args = _codex_config_args_for_permission_profile(
                self._permission_profile
            )
        ...
        # For full-access Feishu mode we instead pass Codex's own config keys
        # (`approval_policy`, `sandbox_mode`) when spawning app-server above.
        # That makes the policy independent of thread/start validation and of
        # whichever model/API login is active for the local Codex account.
        ...
        logger.info(
            "codex app-server thread started: id=%s profile=%s config_overrides=%s cwd=%s",
            self._thread_id[:8],
            self._permission_profile,
            _summarize_codex_config_args(extra_args),
            self._cwd,
        )
        return self._thread_id
    ...
    def _forward_display_event(self, note: dict) -> None:
        """Forward a Codex runtime event to the UI/progress callback."""
        ...
        while time.time() < deadline and not turn_complete:
            ...
            if sreq is not None:
                ...
                for _ in range(8):
                    ...
                    if pending is None:
                        break
                    self._forward_display_event(pending)
            ...
            self._forward_display_event(note)
...
def _codex_config_args_for_permission_profile(profile: str) -> list[str]:
    """Return `codex app-server -c ...` overrides for bridge profiles.

    `thread/start.permissions` is intentionally not used here because recent
    app-server builds validate it against a `[permissions]` table.  For the
    Feishu bridge's full-access mode we instead launch the subprocess with
    Codex's standard config keys, so the policy survives account/API/model
    switches and is decided before any turn starts.
    """
    normalized = str(profile or "").strip().lower().replace("_", "-")
    if normalized in {"full-access", "danger-full-access", "unrestricted", "yolo"}:
        return [
            "-c",
...
def _summarize_codex_config_args(args: list[str]) -> str:
    """Log non-secret config override keys without dumping arbitrary values."""
    if not args:
        return "<none>"
    keys: list[str] = []
    for idx, value in enumerate(args):
        if value != "-c" or idx + 1 >= len(args):
            continue
        key = str(args[idx + 1]).split("=", 1)[0].strip()
        if key:
            keys.append(key)
    return ",".join(keys) if keys else "<custom>"
