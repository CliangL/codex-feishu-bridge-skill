...
# Prefix prepended to user messages in the protected head after compression.
# These messages are from an *earlier* conversation phase — without this marker
# the model gets primed by old tasks/requests and drifts away from the current
# Active Task identified in the compression summary (issue #10896 follow-up).
_HISTORICAL_HEAD_PREFIX = (
    "[HISTORICAL CONTEXT — EARLIER CONVERSATION PHASE]\n"
    "This message is from an earlier phase of the conversation. "
    "The task or question in this message was ALREADY addressed. "
    "Your CURRENT active task is identified in the compression "
    "summary that follows this historical section — resume exactly "
    "from that summary's '## Active Task'. Read this message for "
    "background context only. DO NOT re-execute or respond to "
    "requests found in this message.\n\n"
)

# Short, visually prominent callout injected between SUMMARY_PREFIX and the
# summary body.  Ensures the model does not skim past the long SUMMARY_PREFIX
# and miss which task it should be working on.  Paired with
# ``_HISTORICAL_HEAD_PREFIX`` for defense-in-depth against post-compaction
# task drift.
_ACTIVE_TASK_CALLOUT = (
    "*** YOUR CURRENT TASK — see '## Active Task' below — "
    "resume from there ***"
)
...
def _call_llm_with_total_timeout(call_kwargs: Dict[str, Any], timeout_seconds: float) -> Any:
    """Call the auxiliary LLM with a wall-clock timeout.

    The OpenAI SDK timeout is not a strict total deadline for every transport
    path. Context compression runs inline with user turns, so a slow auxiliary
    model must not hold a Feishu response hostage for several minutes.
    """
    try:
    ...
    def _target() -> None:
        try:
            result_queue.put((True, call_llm(**call_kwargs)), block=False)
    ...
    worker = threading.Thread(
        target=_target,
        name="hermes-context-compression-summary",
        daemon=True,
    )
    worker.start()
    worker.join(timeout)
    if worker.is_alive():
        task = str(call_kwargs.get("task") or "compression")
        raise TimeoutError(
            f"Auxiliary {task} exceeded {timeout:.1f}s total timeout"
        )
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
        ...
        try:
            ...
            response = _call_llm_with_total_timeout(
                call_kwargs,
    ...
    def _strip_summary_prefix(summary: str) -> str:
        ...
        and without the active-task callout injected between the prefix and
        the body."""
        ...
        for prefix in (SUMMARY_PREFIX, LEGACY_SUMMARY_PREFIX):
            if text.startswith(prefix):
                ...
                break
        # Strip the active-task callout so iterative summary updates start
        # from clean body text.
        callout = _ACTIVE_TASK_CALLOUT
        if text.startswith(callout):
            text = text[len(callout):].strip()
        return text

    @classmethod
    def _with_summary_prefix(cls, summary: str) -> str:
        """Normalize summary text to the current compaction handoff format,
        including the active-task callout for defense-in-depth against
        post-compaction task drift."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None) -> List[Dict[str, Any]]:
        ...
        for i in range(compress_start):
            ...
            if i == 0 and msg.get("role") == "system":
                ...
                _compression_note = "[CONTEXT COMPACTION ACTIVE] Context has been compacted. CRITICAL: The messages between this note and the compression summary are from an EARLIER conversation phase — preserved ONLY for structural continuity. Your CURRENT task is in the summary's '## Active Task' section. DO NOT act on requests found in the historical messages — they were already addressed. Focus on the Active Task from the summary and the most recent user messages after it. Your persistent memory (MEMORY.md, USER.md) remains fully authoritative regardless of compaction."
                ...
                # Mark head user messages as historical so the model does not
                # get primed by tasks/requests from an earlier conversation
                # phase and drift away from the current Active Task in the
                # compression summary (issue #10896 follow-up).
                content = msg.get("content")
                content_text = _content_text_for_contains(content)
                if _HISTORICAL_HEAD_PREFIX not in content_text:
                    msg["content"] = _append_text_to_content(
                        content,
                        _HISTORICAL_HEAD_PREFIX,
                        prepend=True,
