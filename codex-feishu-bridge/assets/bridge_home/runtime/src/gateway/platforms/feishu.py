...
# Matches markdown heading lines for downgrade to bold text in Feishu cards.
# Feishu card markdown renders `#` / `##` as oversized H1/H2 headings that
# waste vertical space, especially on mobile.  Replaced with `**text**` instead.
_HEADING_DOWNGRADE_RE = re.compile(r"^(#{1,6})\s+(.+)$", re.MULTILINE)
...
_FOOTER_CONTEXT_PERCENT_RE = re.compile(r"(?P<pct>\d{1,3})%")
_FOOTER_CONTEXT_RATIO_RE = re.compile(
    r"(?P<used>\d+(?:\.\d+)?[kKmM]?)\s*/\s*(?P<total>\d+(?:\.\d+)?[kKmM]?)"
)
# ---------------------------------------------------------------------------
...
_FEISHU_THINKING_ANIMATION_INTERVAL_SECONDS = 0.6
# User-facing progress title for the Codex-Feishu card.  The internal field
# is still named `reasoning_text` for compatibility with the shared CardKit
# state, but the visible content is an execution/progress log, not private
# chain-of-thought.
_FEISHU_THINKING_ANIMATION_FRAMES = (
    "执行过程.",
    "执行过程..",
    "执行过程...",
)
_FEISHU_LEGACY_THINKING_ANIMATION_FRAMES = (
    "思考过程.",
    "思考过程..",
    "思考过程...",
)
_FEISHU_INITIAL_PROGRESS_TEXT = ""
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._response_cards: Dict[str, FeishuResponseCardState] = {}
        self._response_card_messages: Dict[str, str] = {}
        self._response_card_upsert_locks: Dict[str, asyncio.Lock] = {}
        self._cardkit_seq_counters: Dict[str, int] = {}
        self._cardkit_seq_lock = threading.Lock()
        self._cardkit_update_locks: Dict[str, asyncio.Lock] = {}
        self._thinking_animation_tasks: Dict[str, asyncio.Task] = {}
    ...
    @staticmethod
    def _split_card_footer(content: str) -> tuple[str, str]:
        text = str(content or "").strip()
        if "\n\n" not in text:
            return text, ""
        body, tail = text.rsplit("\n\n", 1)
        tail = tail.strip()
        if tail and "\n" not in tail and "·" in tail and len(tail) <= 160:
            return body.strip(), tail
        return text, ""

    @staticmethod
    def _downgrade_card_headings(text: str) -> str:
        """Replace ``# Heading`` → ``**Heading**`` outside code fences.

        Feishu card markdown renders ``#`` / ``##`` / ``###`` as oversized
        headings that waste vertical space, especially on mobile.  Bold text
        conveys the same structure without the giant font.
        """
        if "```" not in text:
            return _HEADING_DOWNGRADE_RE.sub(r"**\2**", text)
        # Split on code fences: odd indices are inside code blocks,
        # even indices are regular text — only downgrade regular text.
        parts = text.split("```")
        for i in range(0, len(parts), 2):
            if i < len(parts):
                parts[i] = _HEADING_DOWNGRADE_RE.sub(r"**\2**", parts[i])
        return "```".join(parts)

    @staticmethod
    def _sanitize_card_markdown(content: str) -> str:
        text = str(content or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        return text or FeishuAdapter._build_thinking_animation_frame(0)

    @staticmethod
    def _parse_footer_context_percent(segment: str) -> Optional[float]:
        text = str(segment or "").strip()
        if not text:
            return None

        percent_match = _FOOTER_CONTEXT_PERCENT_RE.fullmatch(text)
        if percent_match:
            try:
        ...
        ratio_match = _FOOTER_CONTEXT_RATIO_RE.fullmatch(text)
        if not ratio_match:
            return None
        ...
        try:
            used = _parse_compact_number(ratio_match.group("used"))
            total = _parse_compact_number(ratio_match.group("total"))
        except Exception:
            return None
        if total <= 0:
            return None
    ...
    @staticmethod
    def _context_color_for_percent(percent: Optional[float]) -> Optional[str]:
        if percent is None:
            return None
        if percent <= 70.0:
            return "green"
    ...
    def _render_feishu_footer_markdown(self, footer_text: str) -> str:
        footer = str(footer_text or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        if not footer:
            return ""
        ...
        for index, segment in enumerate(segments):
            percent = self._parse_footer_context_percent(segment)
            if percent is None:
                continue
            target_index = index
        ...
        color = self._context_color_for_percent(target_percent)
        if not color:
            return footer
        highlighted = f"<font color='{color}'>{segments[target_index]}</font>"
        segments[target_index] = highlighted
        return " · ".join(segment for segment in segments if segment)

    @staticmethod
    def _merge_reasoning_text(existing: str, incoming: str, *, max_chars: int = 4000) -> str:
    ...
    @staticmethod
    def _build_thinking_animation_frame(frame_index: int = 0) -> str:
        return _FEISHU_THINKING_ANIMATION_FRAMES[
            frame_index % len(_FEISHU_THINKING_ANIMATION_FRAMES)
        ]

    @staticmethod
    def _is_thinking_animation_frame(content: str) -> bool:
    ...
    @staticmethod
    def _response_card_state_message_id(card_id: str, message_id: Optional[Any]) -> str:
        if message_id:
            return str(message_id)
        return f"cardkit:{card_id}"

    def _response_card_key(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        if isinstance(metadata, dict):
            card_key = str(metadata.get("_feishu_card_key") or "").strip()
            if card_key:
                return f"{chat_id}::card::{card_key}"
            session_key = str(metadata.get("_feishu_card_session_key") or "").strip()
            if session_key:
                return f"{chat_id}::session::{session_key}"
            thread_id = str(
    ...
    def _get_response_card(
        self,
        *,
        chat_id: Optional[str] = None,
        metadata: Optional[Dict[str, Any]] = None,
        message_id: Optional[str] = None,
    ) -> tuple[Optional[str], Optional[FeishuResponseCardState]]:
        if message_id:
            card_key = self._response_card_messages.get(str(message_id))
            if card_key:
                return card_key, self._response_cards.get(card_key)
        if chat_id:
            card_key = self._response_card_key(chat_id, metadata)
            return card_key, self._response_cards.get(card_key)
        return None, None

    def _use_single_response_card(
        self,
        *,
        chat_id: str,
        metadata: Optional[Dict[str, Any]] = None,
        ...
        if not isinstance(metadata, dict):
            return False
        return bool(
            metadata.get("_feishu_cron_delivery")
            or metadata.get("_feishu_single_card")
        )

    @staticmethod
    def _build_tool_section(tool_lines: Sequence[str], total_count: int = 0) -> str:
        normalized = [str(line or "").strip() for line in tool_lines if str(line or "").strip()]
        if not normalized:
            return ""
        title = "工具调用"
        if total_count and total_count > len(normalized):
            title = f"{title}（最近 {len(normalized)} / 共 {total_count}）"
        return title + "\n" + "\n".join(f"- {line}" for line in normalized)

    @staticmethod
    def _build_card_summary_text(text: str, max_length: int = 50) -> str:
    ...
    def _plain_text(content: str) -> Dict[str, str]:
        return {"tag": "plain_text", "content": str(content or "")}

    @staticmethod
    def _card_brand(metadata: Optional[Dict[str, Any]] = None) -> str:
        if isinstance(metadata, dict):
            brand = str(metadata.get("_feishu_card_brand") or "").strip()
            if brand:
                return brand[:32]
        return os.getenv("FEISHU_CARD_BRAND", "Hermes").strip() or "Hermes"

    @staticmethod
    def _build_collapsible_panel(
        *,
        title: str,
        content: str,
        expanded: bool = False,
        ...
        return {
            ...
            "expanded": expanded,
            "header": {
                "title": FeishuAdapter._plain_text(title),
                "icon": {
                    "tag": "standard_icon",
    ...
    @staticmethod
    def _compact_notification_lines(content: str) -> tuple[str, List[str], List[str]]:
        text = str(content or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        if not text:
            return "Codex 通知" if os.getenv("FEISHU_CARD_BRAND") == "Codex" else "Hermes 通知", [], []
        lines = [line.strip(" \t-•") for line in text.splitlines() if line.strip()]
        title = lines[0] if lines else (os.getenv("FEISHU_CARD_BRAND", "Hermes").strip() + " 通知")
        if title.startswith("#"):
            title = title.lstrip("#").strip()
        title = re.sub(r"^(Cronjob Response:|Cron Job:)\s*", "", title, flags=re.IGNORECASE).strip()
        ...
        detail: List[str] = []
        for line in body_lines:
            if len(important) < 3 and len(line) <= 180:
                important.append(line)
            else:
                detail.append(line)
        if not important and lines:
            important = [lines[0]]
        return title or (os.getenv("FEISHU_CARD_BRAND", "Hermes").strip() + " 通知"), important, detail

    @staticmethod
    def _format_tool_elapsed_ms(value: int) -> str:
        seconds = max(0, int(value or 0)) / 1000.0
        if seconds < 60:
            return f"{seconds:.1f} 秒".replace(".0 秒", " 秒")
        minutes = int(seconds // 60)
        rem = int(seconds % 60)
        return f"{minutes} 分 {rem:02d} 秒"

    def _build_single_response_card_payload(
        self,
        *,
        content: str,
        metadata: Optional[Dict[str, Any]] = None,
        reasoning_text: str = "",
        tool_lines: Optional[Sequence[str]] = None,
        tool_total_count: int = 0,
        tool_elapsed_ms: int = 0,
        footer_text: str = "",
        finalized: bool = False,
    ) -> str:
        raw_body = str(content or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        brand = self._card_brand(metadata)
        has_real_body = bool(raw_body) and not self._is_thinking_animation_frame(raw_body)
        body = self._sanitize_card_markdown(content) if has_real_body else ""
        thinking_frame = (
            raw_body
            if (not has_real_body and self._is_thinking_animation_frame(raw_body))
            else self._build_thinking_animation_frame(0)
        )
        raw_footer = str(footer_text or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        rendered_footer = self._render_feishu_footer_markdown(raw_footer)

        elements: List[Dict[str, Any]] = []
        normalized_tool_lines = [
            str(line or "").strip() for line in (tool_lines or []) if str(line or "").strip()
        ]
        ...
        rendered_body = (
            self._downgrade_card_headings(body)
            if has_real_body
            else (thinking_frame if show_idle_placeholder else "")
        )
        if normalized_tool_lines:
            display_lines = normalized_tool_lines[-5:]
            title = f"🛠️ 工具调用 · {self._format_tool_elapsed_ms(tool_elapsed_ms)} · {tool_total_count or len(normalized_tool_lines)} 步"
            details = "\n".join(f"- {line}" for line in display_lines)
            elements.append(
                self._build_collapsible_panel(
                    title=title,
                    content=details,
                    expanded=False,
                )
        ...
        if reasoning_text and not has_real_body:
            reasoning_body = self._downgrade_card_headings(self._sanitize_card_markdown(reasoning_text))
            elements.append(
                {
                    "tag": "markdown",
                    "content": f"**{thinking_frame}**\n{reasoning_body}",
                }
            )

        if rendered_body and (has_real_body or not reasoning_text):
            elements.append(
                {
                    "tag": "markdown",
        ...
        rendered_summary_parts: List[str] = []
        if rendered_body and has_real_body:
            rendered_summary_parts.append(rendered_body)
        elif reasoning_text and not has_real_body:
            rendered_summary_parts.append(reasoning_text)
        elif rendered_body:
            rendered_summary_parts.append(rendered_body)
        if normalized_tool_lines and not rendered_summary_parts:
            rendered_summary_parts.append(normalized_tool_lines[-1])
        if rendered_footer:
            rendered_summary_parts.append(rendered_footer)
        rendered = "\n".join(part for part in rendered_summary_parts if str(part).strip())
        summary = self._build_card_summary_text(rendered)
        payload = {
            "schema": "2.0",
            "config": {
            ...
            "body": {"elements": elements or [{"tag": "markdown", "content": "处理中..."}]},
        }
        if summary:
            payload["config"]["summary"] = {"content": summary}
        return json.dumps(payload, ensure_ascii=False)

    def _normalize_cron_notification_content(self, content: str) -> str:
        text = self._sanitize_card_markdown(content)
    ...
    @staticmethod
    def _split_cron_notification_blocks(content: str) -> List[str]:
        text = str(content or "").replace("\r\n", "\n").replace("\r", "\n").strip()
        if not text:
            return []
    ...
    def _build_cron_notification_card_payload(
        self,
        *,
        content: str,
        metadata: Optional[Dict[str, Any]] = None,
    ) -> str:
        normalized = self._normalize_cron_notification_content(content)
        meta = metadata if isinstance(metadata, dict) else {}
        brand = self._card_brand(meta)
        job_name = str(meta.get("_cron_job_name") or "").strip()
        blocks = self._split_cron_notification_blocks(normalized)
        title = brand
        if job_name:
            title = job_name[:48].rstrip() + ("..." if len(job_name) > 48 else "")
        payload = {
            "schema": "2.0",
            "config": {
                "wide_screen_mode": True,
                "update_multi": True,
                "locales": ["zh_cn", "en_us"],
                "summary": {"content": self._build_card_summary_text(normalized, max_length=80)},
            },
            "header": {
                "title": self._plain_text(title or f"{brand} 通知"),
                "subtitle": self._plain_text("通知"),
                "template": "green",
                "text_tag_list": [
                    {"tag": "text_tag", "text": self._plain_text("完成"), "color": "green"}
                ],
            },
            "body": {"elements": []},
        }
        for block in blocks:
            payload["body"]["elements"].append(
                {"tag": "markdown", "content": self._downgrade_card_headings(block)}
            )
        if not payload["body"]["elements"]:
            payload["body"]["elements"].append(
                {
                    "tag": "markdown",
                    "content": self._build_card_summary_text(normalized, max_length=140) or "任务已完成",
                }
            )
        return json.dumps(payload, ensure_ascii=False)

    @staticmethod
    def _is_invalid_card_reference_error(error_text: str) -> bool:
        text = str(error_text or "").lower()
        return "cardid is invalid" in text or "errcode: 11310" in text

    async def _send_cron_notification_card(
        self,
        *,
        chat_id: str,
        content: str,
        ...
        payload = self._build_cron_notification_card_payload(content=content, metadata=metadata)
        last_result = SendResult(success=False, error="CardKit create failed")
        for attempt in range(2):
            card_id = await self._create_cardkit_card_entity(payload)
            if not card_id:
                return last_result
            ...
            last_result = self._finalize_send_result(response, "send failed")
            if last_result.success:
                return last_result
            if not self._is_invalid_card_reference_error(last_result.error):
                return last_result
            if attempt == 0:
                logger.warning(
                    "[Feishu] CardKit reference rejected with invalid card_id; recreating cron card and retrying once"
                )
                await asyncio.sleep(0.5)
        return last_result

    async def _update_interactive_card_message(self, message_id: str, payload: str) -> SendResult:
    ...
    async def _create_cardkit_card_entity(self, card_payload: str) -> Optional[str]:
        if not self._client:
            return None
        try:
            from lark_oapi.api.cardkit.v1 import CreateCardRequest, CreateCardRequestBody

            body = CreateCardRequestBody.builder().type("card_json").data(card_payload).build()
            ...
            if getattr(response, "code", None) == 0 and getattr(response, "data", None):
            ...
            logger.warning("[Feishu] CardKit create exception: %s", exc)
        return None

    def _get_cardkit_update_lock(self, card_id: str) -> asyncio.Lock:
        lock = self._cardkit_update_locks.get(card_id)
        if lock is None:
            lock = asyncio.Lock()
            self._cardkit_update_locks[card_id] = lock
        return lock

    def _get_response_card_upsert_lock(self, card_key: str) -> asyncio.Lock:
        lock = self._response_card_upsert_locks.get(card_key)
        if lock is None:
            lock = asyncio.Lock()
            self._response_card_upsert_locks[card_key] = lock
        return lock

    def _next_cardkit_sequence(self, key: str) -> int:
        with self._cardkit_seq_lock:
            current = self._cardkit_seq_counters.get(key, 0) + 1
            self._cardkit_seq_counters[key] = current
            return current

    def _start_thinking_animation(self, card_key: str) -> None:
        existing = self._thinking_animation_tasks.get(card_key)
        if existing and not existing.done():
            return
        self._thinking_animation_tasks[card_key] = asyncio.create_task(
            self._run_thinking_animation(card_key)
        )

    def _stop_thinking_animation(self, card_key: str) -> None:
        task = self._thinking_animation_tasks.pop(card_key, None)
        if task and not task.done():
            task.cancel()

    async def _run_thinking_animation(self, card_key: str) -> None:
        frame_index = 1
        try:
            while True:
                await asyncio.sleep(_FEISHU_THINKING_ANIMATION_INTERVAL_SECONDS)
                state = self._response_cards.get(card_key)
                if (
                    not state
                    or state.finalized
                    or not state.card_id
                    or str(state.content or "").strip()
                    and not self._is_thinking_animation_frame(state.content)
                ):
                    return
                state.content = self._build_thinking_animation_frame(frame_index)
                payload = self._build_single_response_card_payload(
                    content=state.content,
                    metadata=state.metadata,
                    reasoning_text=state.reasoning_text,
                    tool_lines=state.tool_lines,
                    tool_total_count=state.tool_total_count,
                    tool_elapsed_ms=state.tool_elapsed_ms,
                    footer_text=state.footer_text,
                    finalized=False,
                )
                await self._cardkit_update_full_card(state.card_id, payload)
                frame_index += 1
        except asyncio.CancelledError:
            return
        except Exception as exc:
            logger.debug("[Feishu] Thinking animation stopped: %s", exc)

    async def _cardkit_update_full_card(self, card_id: str, card_payload: str) -> bool:
        if not self._client:
            return False
        try:
            from lark_oapi.api.cardkit.v1 import Card as CardKitCard, UpdateCardRequest, UpdateCardRequestBody
        except Exception as exc:
            logger.warning("[Feishu] CardKit SDK import failed: %s", exc)
            return False

        lock = self._get_cardkit_update_lock(card_id)
        async with lock:
            last_code = None
            last_msg = ""
            for attempt in range(_CARDKIT_RETRY_ATTEMPTS):
                try:
                    sequence = self._next_cardkit_sequence(card_id)
                    card = CardKitCard.builder().type("card_json").data(card_payload).build()
                    body = (
                    ...
                    last_msg = getattr(response, "msg", "") or ""
                    logger.warning(
                        "[Feishu] CardKit card.update failed: code=%s msg=%s",
                        last_code,
                        last_msg,
                    )
                except Exception as exc:
                    ...
                    last_msg = str(exc)
                    logger.warning(
                        "[Feishu] CardKit card.update exception (attempt %d/%d): %s",
                        attempt + 1,
                        _CARDKIT_RETRY_ATTEMPTS,
                        exc,
                    )
                if attempt < _CARDKIT_RETRY_ATTEMPTS - 1:
                    await asyncio.sleep(2 ** attempt)
            logger.warning(
                "[Feishu] CardKit card.update exhausted retries: code=%s msg=%s",
                last_code,
                last_msg,
            )
            return False
    ...
    @staticmethod
    def _is_card_reference_payload(payload: str) -> bool:
        try:
        ...
        return isinstance(data, dict) and bool(str(data.get("card_id") or "").strip())

    async def _upsert_single_response_card(
        self,
        *,
        chat_id: str,
        ...
        tool_lines: Optional[Sequence[str]] = None,
        tool_total_count: Optional[int] = None,
        tool_elapsed_ms: Optional[int] = None,
        footer_text: Optional[str] = None,
        finalize: bool = False,
    ) -> SendResult:
        card_key = self._response_card_key(chat_id, metadata)
        async with self._get_response_card_upsert_lock(card_key):
            return await self._upsert_single_response_card_unlocked(
                chat_id=chat_id,
                content=content,
                reply_to=reply_to,
                metadata=metadata,
    ...
    async def _upsert_single_response_card_unlocked(
        self,
        *,
        chat_id: str,
        content: Optional[str],
        ...
        card_key = self._response_card_key(chat_id, metadata)
        state = self._response_cards.get(card_key)
        if state and state.finalized and not finalize:
            if isinstance(metadata, dict) and metadata.get("_feishu_card_session_key"):
                state.finalized = False
            else:
                self._response_card_messages.pop(state.message_id, None)
                self._response_cards.pop(card_key, None)
                self._stop_thinking_animation(card_key)
                state = None

        body_text = content
        extracted_footer = ""
        if body_text is not None:
            body_text, extracted_footer = self._split_card_footer(body_text)
        effective_footer = footer_text if footer_text is not None else extracted_footer

        next_content = (body_text if body_text is not None else (state.content if state else ""))
        has_real_body = bool(next_content.strip()) and not self._is_thinking_animation_frame(next_content)
        if has_real_body:
            next_reasoning = ""
            next_tool_lines = list(tool_lines) if tool_lines is not None else (list(state.tool_lines) if state else [])
            next_tool_total = (
                int(tool_total_count)
                if tool_total_count is not None
                else (state.tool_total_count if state else len(next_tool_lines))
            )
            next_tool_elapsed = (
                int(tool_elapsed_ms)
                if tool_elapsed_ms is not None
                else (state.tool_elapsed_ms if state else 0)
            )
        else:
            if reasoning_text is not None:
                next_reasoning = self._merge_reasoning_text(
                    state.reasoning_text if state else "",
                    reasoning_text,
                )
            else:
                next_reasoning = state.reasoning_text if state else ""
            next_tool_lines = list(tool_lines) if tool_lines is not None else (list(state.tool_lines) if state else [])
            next_tool_total = (
                int(tool_total_count)
                if tool_total_count is not None
        ...
        next_footer = effective_footer if effective_footer is not None else (state.footer_text if state else "")
        if has_real_body:
            self._stop_thinking_animation(card_key)
        ...
        payload = self._build_single_response_card_payload(
            ...
            tool_lines=next_tool_lines,
            tool_total_count=next_tool_total,
            tool_elapsed_ms=next_tool_elapsed,
            footer_text=next_footer,
            finalized=finalize,
        )
        ...
        if state is None:
            ...
            if not result.success:
                return result
            state_message_id = self._response_card_state_message_id(card_id, result.message_id)
            state = FeishuResponseCardState(
                message_id=state_message_id,
                card_id=str(card_id),
                actual_message_id=str(result.message_id) if result.message_id else "",
                reply_to=reply_to,
            ...
            self._response_cards[card_key] = state
            self._response_card_messages[state_message_id] = card_key
            if result.message_id:
                self._response_card_messages[str(result.message_id)] = card_key
            if not has_real_body and not finalize:
                self._start_thinking_animation(card_key)
            logger.info(
                "[Feishu] Single-card sent chat_id=%s message_id=%s card_id=%s finalized=%s",
                chat_id,
                state_message_id,
                card_id,
                finalize,
        ...
        if state.card_id:
            if not finalize and not has_real_body:
                self._stop_thinking_animation(card_key)
            state.reply_to = reply_to or state.reply_to
            state.metadata = dict(metadata or state.metadata)
            state.content = next_content
            state.reasoning_text = next_reasoning
            state.tool_lines = next_tool_lines
            state.tool_total_count = next_tool_total
            state.tool_elapsed_ms = next_tool_elapsed
            state.footer_text = next_footer
            state.finalized = finalize
            updated = await self._cardkit_update_full_card(state.card_id, payload)
            if not updated:
                return SendResult(success=False, error="CardKit update failed", message_id=state.message_id)
            result = SendResult(success=True, message_id=state.message_id)
            if finalize or (next_content.strip() and not self._is_thinking_animation_frame(next_content)):
                self._stop_thinking_animation(card_key)
            elif not has_real_body:
                self._start_thinking_animation(card_key)
            logger.info(
                "[Feishu] Single-card updated chat_id=%s message_id=%s card_id=%s finalized=%s",
                chat_id,
                state.message_id,
                state.card_id,
                finalize,
        ...
        result = await self._update_interactive_card_message(state.message_id, payload)
        if not result.success:
            return result
        state.reply_to = reply_to or state.reply_to
        state.metadata = dict(metadata or state.metadata)
        ...
        if finalize:
            self._stop_thinking_animation(card_key)
        logger.info(
            "[Feishu] Single-card edited chat_id=%s message_id=%s finalized=%s",
            chat_id,
            state.message_id,
            finalize,
        )
    ...
    async def send(
        ...
        if self._use_single_response_card(chat_id=chat_id, metadata=metadata):
            try:
                result = await self._upsert_single_response_card(
                    chat_id=chat_id,
                    content=content,
                    reply_to=reply_to,
                    metadata=metadata,
                ...
                if result.success:
                    return result
                logger.warning("[Feishu] Single-card send failed: %s", result.error)
                if isinstance(metadata, dict) and metadata.get("_feishu_cron_delivery"):
                    return result
            except Exception as exc:
                logger.warning("[Feishu] Single-card send failed: %s", exc)
                if isinstance(metadata, dict) and metadata.get("_feishu_cron_delivery"):
                    return SendResult(success=False, error=str(exc))
            if isinstance(metadata, dict) and metadata.get("_feishu_single_card"):
                metadata = {k: v for k, v in metadata.items() if k != "_feishu_single_card"}
    ...
    async def send_tool_progress_card(
        self,
        chat_id: str,
        reply_to: Optional[str] = None,
        metadata: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            from gateway.tool_use_trace import ToolUseTraceStore

            session_key = get_session_env("HERMES_SESSION_KEY", "")
            traces = ToolUseTraceStore.get_traces(session_key) if session_key else None
            tool_lines = [str(step.get("line") or "").strip() for step in (traces or {}).get("steps", []) if str(step.get("line") or "").strip()]
            total_count = int((traces or {}).get("total_count", 0) or 0)
            elapsed_ms = int((traces or {}).get("elapsed_ms", 0) or 0)

            card_metadata = dict(metadata or {})
            card_metadata["_feishu_single_card"] = True

            if not tool_lines and not reasoning_text:
                card_key, state = self._get_response_card(chat_id=chat_id, metadata=card_metadata)
                if not state:
                    return await self._upsert_single_response_card(
                        chat_id=chat_id,
                        content=None,
                        reply_to=reply_to,
                        ...
                        reasoning_text=None,
                        tool_lines=None,
                        tool_total_count=None,
                        tool_elapsed_ms=None,
                        finalize=False,
                    )
                return SendResult(success=True, message_id=state.message_id)

            tool_lines_arg: Optional[List[str]] = tool_lines if tool_lines else None
            total_count_arg: Optional[int] = total_count if tool_lines else None
            elapsed_ms_arg: Optional[int] = elapsed_ms if tool_lines else None
            return await self._upsert_single_response_card(
                chat_id=chat_id,
                content=None,
                reply_to=reply_to,
                metadata=card_metadata,
                reasoning_text=reasoning_text,
                tool_lines=tool_lines_arg,
                tool_total_count=total_count_arg,
                tool_elapsed_ms=elapsed_ms_arg,
                finalize=False,
            )
        except Exception as exc:
            logger.debug("[Feishu] Tool progress card error: %s", exc)
            return SendResult(success=False, error=str(exc))

    async def append_runtime_footer_card(
        self,
        chat_id: str,
        footer_text: str,
        metadata: Optional[Dict[str, Any]] = None,
        ...
        if not footer:
            return SendResult(success=False, error="Empty footer")
        try:
            card_metadata = dict(metadata or {})
            card_metadata["_feishu_single_card"] = True
            return await self._upsert_single_response_card(
            ...
            logger.debug("[Feishu] Runtime footer card append error: %s", exc)
            return SendResult(success=False, error=str(exc))
    ...
    async def edit_message(
        ...
        *,
        ...
        _, state = self._get_response_card(chat_id=chat_id, message_id=message_id)
        if state is not None:
            try:
                card_metadata = dict(state.metadata or {})
                card_metadata["_feishu_single_card"] = True
                result = await self._upsert_single_response_card(
                    chat_id=chat_id,
                    content=content,
                ...
                if result.success:
                    return result
                logger.warning("[Feishu] Single-card edit failed; falling back to plain edit: %s", result.error)
            except Exception as exc:
                logger.warning("[Feishu] Single-card edit failed; falling back to plain edit: %s", exc)
    ...
    async def _send_raw_message(
        ...
        *,
        ...
        is_card_reference = (
            msg_type == "interactive" and self._is_card_reference_payload(payload)
        )
        # Raw interactive payloads are flaky through reply/create-in-thread.
        # CardKit references are stable and can use the normal reply path.
        if msg_type == "interactive" and not is_card_reference:
        ...
        if _thread_id and not is_card_reference:
    ...
    def _extract_response_field(response: Any, field_name: str) -> Any:
        ...
        if field_name == "message_id":
            for alias in ("open_message_id", "msg_id"):
                value = getattr(data, alias, None)
                if value is not None:
                    return value
        return None
