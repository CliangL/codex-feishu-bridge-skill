...
class GatewayStreamConsumer:
    ...
    def _current_visible_text(self) -> str:
        """Return the best-effort full visible text for the current message.

        Prefer ``_accumulated`` because it holds the full final body even when
        the adapter has rotated to a continuation message and ``_last_sent_text``
        no longer mirrors the whole visible transcript.
        """
        text = self._accumulated or self._last_sent_text or ""
        if self.cfg.cursor and text.endswith(self.cfg.cursor):
            text = text[:-len(self.cfg.cursor)]
        return self._clean_for_display(text).rstrip()
    ...
    async def append_footer(self, footer_text: str) -> bool:
        """Append a footer to the current streamed message in-place.

        Returns True when the footer was successfully merged into the visible
        final message. Callers should fall back to a separate trailing send
        when this returns False.
        """
        footer = self._clean_for_display(footer_text or "").strip()
        if not footer:
            return False
        ...
        body = self._current_visible_text().strip()
        if not body:
            return False

        merged = body if body.endswith(footer) else f"{body}\n\n{footer}"
        try:
            ...
            logger.debug("append_footer edit failed: %s", e)
            return False
